...
# deping on the sign of "length") used.
#
...
#
    ...
    while i < abs(length):                                      # while not finished
        ...
        while 1:
            while ((f2 > f1+RHO*z1*d1) or (d2 > -SIG*d1)) and (M > 0): 
            ...
            B = 3*(f3-f2)-z3*(d3+2*d2)
            z2 = -d2*z3*z3/(B+((B*B-A*d2*z3*z3)**0.5))        # num. error possible - ok!
            if ~np.isreal(z2) or math.isnan(z2) or math.isinf(z2) or z2 < 0: # num prob or wrong sign?
